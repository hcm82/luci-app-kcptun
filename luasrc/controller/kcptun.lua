...
function action_check(type)
	local type = type and type ~= "" and type or kcptun
	...
	if obj then
		...
		if json.version ~= "" then
			...
			json.needs_update = compare_versions(old_version, "<", json.version)
		end
	end
...
function action_update(type)
	...
	if type and type ~= "" then
		code = sys.call("sh /usr/lib/%s/%s_update.sh update" %{ kcptun, type })
	end

...
get_latest_kcptun_version() {
	...
	if [ -z "$file_tree" ]; then
		_err "Can't determine ARCH. Please select manually."
		retval=11
		return 1
	fi
	...
	if json_is_a assets array; then
		...
		for asset in $assets; do
			...
			if [ -n "$browser_download_url" ]; then
				if ( echo "$browser_download_url" | grep -qw "$file_tree" ); then
					kcptun_download_url="$browser_download_url"
					break
				fi
			elif [ -n "$name" ] && [ -n "$tag_name" ]; then
				if ( echo "$name" | grep -qw "$file_tree" ); then
...
update_kcptun() {
	...
	new_file="$(ls "$extract_path/client"* | head -n 1)"
